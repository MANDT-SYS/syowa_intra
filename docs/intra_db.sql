...
--     前提として、ユーザー、部署は外部DBを使用している為、ユーザー、部署関連のcolumnは外部キーでつなぐようなことはない。
        ...
        -- カラムコメント
        COMMENT ON COLUMN document_categories.id IS '主キー';
        ...
        -- カラムコメント
        COMMENT ON COLUMN documents.id IS '主キー';
        ...
        CREATE TABLE revisions (
            -- 主キー：版を一意に識別するID
            id BIGINT PRIMARY KEY DEFAULT nextval('revisions_id_seq'),
